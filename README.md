...
书接上回
  ...
  理论上讲，如果我们这样写a = 3,b = 3，这两个对象3分属不同内存，但python内部作了优化，缓存了不变的对象对其复用。
  ...
  依然如上边三行代码，在123和python都被放弃引用后，如果它们们有被其它变量引用，python内部会自动把它们回收来达到释放内存的效果，这得益于头部信息的第二个--计数器，当计数器为零是，内部会认为当前对象已无用，自动将其释放到自由空间中去。（我自动脑部以下对话）
