...
书接上回
  ...
  *********************(居然可以坚持到第六次学习的 学习第六天)
  ...
  学习过程中有一个值得注意的小插曲是使用.keys()这个方法获取键的时候，在python早些版本的时候，用这个方法获取的对象直接就是列表，而在3.x以后，这个方法获取的是叫做dict_keys这么个类型，为什么能注意到这个点，因为在尝试他书中这部分方法的时候，我是提前用powershell敲过的，得出来的确实也是报错，之后书中也提到了这一点，在3.0之后，用.keys()得到的是一个可迭代的对象，并不是真正的列表，想要获得列表，可以在用list，将这个迭代对象变为列表，即list(dict.keys())，这样就得到键的列表了。不过我在自己本机操作的时候，用了list这个方法后依然报错，后来不用powershell，用pycharm又运行了一遍，这次就没问题了，我百度了半天我出现的这种问题，换过了各种描述方式，没有找到我的这种情况，所以暂定为我自己电脑的问题。
